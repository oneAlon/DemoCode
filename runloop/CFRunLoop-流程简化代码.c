...
struct __CFRunLoopMode {
    ...
    CFStringRef _name;// 运行模式的名称, 例如`AFN...`
    Boolean _stopped;
    char _padding[3];
...
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    ...
    if (!__CFRunLoops) {
        ...
        // 将主线程作为key, runloop作为value存入dict中
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    int32_t retVal = 0;
    do {
        // 2.通知观察者准备开始处理Timer源事件
        __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeTimers);
        // 3.通知观察者准备开始处理Source源事件
        __CFRunLoopDoObservers(rl, rlm, kCFRunLoopBeforeSources);
        
        // 4.执行提交到runloop中的block
        __CFRunLoopDoBlocks(rl, rlm);

        // 5.执行Sources0的源事件, 处理完Source0可能再次处理block
        Boolean sourceHandledThisLoop = __CFRunLoopDoSources0(rl, rlm, stopAfterHandle);
        if (sourceHandledThisLoop) {
            __CFRunLoopDoBlocks(rl, rlm);
        }
        // 表示Sources0的源事件是否执行完成
        Boolean poll = sourceHandledThisLoop || (0ULL == timeout_context->termTSR);

        // 6.监测Source1, 如果端口有事件就跳转至handle_msg
        ...
        // 8.进入循环不断读取端口信息, 如果有信息就唤醒runloop
        do {
